    ...
    unsafe { 
...

        {
            ...
            if num_of_presses_on_page == 4 {
                ...
                draw2d::square::square_middle(Vec2::new(1920. / 2., 1080. / 2.), 400., 400., 
...
async fn run(window: Window) {
    ...
    while window.update_screen().await {
        ...
        match page {
            ...
            9 => minecraft_run(&window).await,
            10 => end(&window, rngs.clone(), fonts.clone()).await,
            _other => break
        };
...
async fn minecraft_run(window: &Window){   
    let (
        textures, 
        default_cam, 
        mut cam3d, 
